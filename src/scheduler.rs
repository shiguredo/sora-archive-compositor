...
fn sync_channel_size() -> usize {
    let size = std::env::var("SORA_ARCHIVE_COMPOSITOR_SYNC_CHANNEL_SIZE")
        .ok()
    ...
    fn process_input(&mut self) -> crate::Result<bool> {
        ...
        for &stream_id in &self.awaiting_input_stream_ids {
            let rx = self
                .input_stream_rxs
                .get(&stream_id)
                .ok_or_else(|| crate::Error::new(format!("unknown input stream: {stream_id:?}")))?;
            match rx.try_recv() {
    ...
    fn process_output(&mut self) -> crate::Result<bool> {
        ...
        if let Some((stream_id, mut i, sample)) = self.output_sample.take() {
            let txs = self.output_stream_txs.get_mut(&stream_id).ok_or_else(|| {
                crate::Error::new(format!("unknown output stream: {stream_id:?}"))
            })?;
            while i < txs.len() {
...
impl Scheduler {
    ...
    fn spawn(mut self) -> crate::Result<SchedulerHandle> {
        ...
        for task in &mut self.tasks {
            ...
            let i = thread_costs
                ...
                .ok_or_else(|| crate::Error::new("no worker threads available"))?
                .0;
    ...
    pub fn run(self) -> crate::Result<Stats> {
    ...
    pub fn run_timeout(self, timeout: Duration) -> crate::Result<(bool, Stats)> {
...
impl TaskRunner {
    ...
    fn run_one(&mut self) {
        ...
        while i < self.tasks.len() {
            ...
            // 空 poll (`Ok(false)`) は処理時間に入れない。Pending を含む実処理 (`Ok(true)`) だけ加算する
