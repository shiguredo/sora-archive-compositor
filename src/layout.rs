...
use crate::{
    ...
    metadata::{ArchiveMetadata, ContainerFormat, SourceId, SourceInfo},
...
impl Layout {
    ...
    pub fn video_bitrate_bps(&self) -> usize {
        // 2026.2.0-canary.0 の shiguredo_libvpx は target_bitrate < 1000 bps を拒否するため、
        // 映像ソースが 0 件で自動計算値が 0 になった場合でも最低 1000 bps を返してエンコーダー生成を通す。
        // (この場合はそもそもエンコード対象のフレームが 0 件なので、値そのものは出力に影響しない)
        self.video_bitrate
...
/// `video_layout` を 1 回の走査でパースし、リージョン名の重複も検査する
fn parse_video_layout<'text, 'raw>(
    value: nojson::RawJsonValue<'text, 'raw>,
) -> Result<BTreeMap<String, RawRegion>, nojson::JsonParseError> {
    let mut region_names = BTreeSet::new();
    let mut video_layout = BTreeMap::new();
    for (name, region_value) in value.to_object()? {
        let region_name = name.to_unquoted_string_str()?.into_owned();
        if !region_names.insert(region_name.clone()) {
            return Err(name.invalid("duplicate region name"));
        }
        video_layout.insert(region_name, region_value.try_into()?);
    }
    Ok(video_layout)
}
...
impl<'text, 'raw> TryFrom<nojson::RawJsonValue<'text, 'raw>> for RawLayout {
    ...
    fn try_from(value: nojson::RawJsonValue<'text, 'raw>) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            video_layout: object
                .get_with("video_layout", parse_video_layout)?
                .unwrap_or_default(),
...
impl FromStr for Resolution {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let Some((Ok(width), Ok(height))) = s.split_once('x').map(|(w, h)| (w.parse(), h.parse()))
            ...
            return Err(format!("invalid resolution: {s}"));
        };

        Self::new(width, height).map_err(|e| format!("invalid resolution: {}", e.reason))
    }
}
...
impl<'text, 'raw> TryFrom<nojson::RawJsonValue<'text, 'raw>> for Resolution {
    ...
    fn try_from(value: nojson::RawJsonValue<'text, 'raw>) -> Result<Self, Self::Error> {
        ...
        Self::from_str(&s).map_err(|e| value.invalid(e))
    }
}
...
// ソース JSON に対応する .webm / .mp4 が存在するかを確認する。
// 存在有無だけを見ており、サイズや中身は見ない。
// そのため testdata/source_timestamps/ や testdata/trim/ の 0 バイト .webm
// （存在チェック専用のプレースホルダ）でも true になる。
...
fn glob(path: PathBuf) -> crate::Result<Vec<PathBuf>> {
    ...
    let wildcard_name = path
        .file_name()
        .and_then(|name| name.to_str())
        .ok_or_else(|| crate::Error::new(format!("invalid file name: {}", path.display())))?;

    let parent = path
        .parent()
        .ok_or_else(|| crate::Error::new(format!("no parent directory: {}", path.display())))?;
    if !parent.exists() {
        return Err(crate::Error::new(format!(
...
impl AggregatedSourceInfo {
    ...
    // 同一 connection の一括録画と分割録画の media_paths を統合する。
    // 時間区間が重なる場合は開始時刻が早い方を残す。開始が同じなら終了が遅い方を残す。
    // 通常の分割録画では重なりは起きない想定で、コーナーケースの優先規則を固定しているだけである。
    pub fn merge_overlapping_sources(&mut self) -> crate::Result<()> {
        let mut sources_by_timespan: Vec<_> = self.media_paths.iter().collect();

        // 開始時刻昇順。同時刻なら終了が遅い方（長い方）を先にする
...
mod tests {
    ...
    fn test_duplicate_region_name() -> crate::Result<()> {
        ...
        let e = json
            ...
            .err()
            .ok_or_else(|| crate::Error::new("パースエラーを期待したが Ok だった"))?;
    ...
    fn test_merge_overlapping_sources_complete_containment() -> crate::Result<()> {
        ...
        // 開始が早い long (0-20) が残り、内側の short (5-15) は除外される
    ...
    fn test_merge_overlapping_sources_partial_overlap() -> crate::Result<()> {
        ...
        assert_eq!(aggregated.start_timestamp, Duration::from_secs(0));
        assert_eq!(aggregated.stop_timestamp, Duration::from_secs(20)); // update で設定された最大値
    ...
    fn test_merge_overlapping_sources_identical_duration() -> crate::Result<()> {
        ...
        assert_eq!(aggregated.start_timestamp, Duration::from_secs(0));
        assert_eq!(aggregated.stop_timestamp, Duration::from_secs(15)); // update で設定された最大値
    ...
    fn test_merge_overlapping_sources_multiple_overlaps() -> crate::Result<()> {
        ...
        // - source1 (0-30) が最初に処理される (開始時刻が最も早い)
        // - source2,3 は source1 の終了時刻 30 より前に開始するため除外
        // - source4 (40-50) は source1 の終了時刻 30 より後に開始するため残る
        assert_eq!(aggregated.media_paths.len(), 2);
        assert!(
            aggregated
                .media_paths
    ...
    fn test_merge_overlapping_sources_sequential() -> crate::Result<()> {
        ...
        let source3 = create_test_source_info(Duration::from_secs(21), Duration::from_secs(30)); // 1 秒の隙間
