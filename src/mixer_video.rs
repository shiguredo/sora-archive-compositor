...
use crate::{
    ...
    video::{FrameRate, VideoFormat, VideoFrame, video_frame_duration_addition_fits},
};
...
struct ResizeCachedVideoFrame {
    ...
    resized: Vec<((EvenUsize, EvenUsize), VideoFrame)>, // (width, height) => リサイズ後のフレーム
...
impl Canvas {
    ...
    fn draw_frame(&mut self, position: PixelPosition, frame: &VideoFrame) -> crate::Result<()> {
        ...
        if frame.width > self.width.get() {
            return Err(crate::Error::new(
                "assertion failed: frame.width <= self.width.get()",
            ));
        }
        if frame.height > self.height.get() {
            return Err(crate::Error::new(
                "assertion failed: frame.height <= self.height.get()",
            ));
        }
        ...
        // data 長が width / height に足りないフレームはここで落とす
        let (y_data, u_data, v_data) = frame
            .as_yuv_planes()
...
impl VideoMixer {
    ...
    fn frames_to_timestamp(&self, frames: u64) -> Duration {
        Duration::from_secs(frames * self.spec.frame_rate.denominator.get() as u64)
    ...
    fn record_gap_extension(&mut self, duration: Duration) {
        self.stats.total_extended_video_frame_count.add(1);
        self.stats.total_output_video_frame_duration.add(duration);
    }

    /// ギャップ延長 tick を hold に反映する。上限超過で分割が必要な場合は flush 対象を返す。
    fn extend_gap_hold(&mut self, tick_duration: Duration) -> Option<VideoFrame> {
        let last_frame = self.last_mixed_frame.as_mut()?;
        if video_frame_duration_addition_fits(last_frame.duration, tick_duration) {
            last_frame.duration += tick_duration;
            self.record_gap_extension(tick_duration);
            return None;
        }
        Some(self.split_gap_hold(tick_duration))
    }

    /// hold の duration が上限に達したため、同一画素の新 hold に引き継いで flush する
    fn split_gap_hold(&mut self, tick_duration: Duration) -> VideoFrame {
        let flushed = self
            .last_mixed_frame
            .take()
            .expect("last_mixed_frame must exist when splitting gap hold");
        let mut new_hold = flushed.clone();
        new_hold.timestamp = flushed.timestamp + flushed.duration;
        new_hold.duration = tick_duration;
        self.last_mixed_frame = Some(new_hold);
        self.record_gap_extension(tick_duration);
        flushed
    }
...
impl MediaProcessor for VideoMixer {
    ...
    fn process_input(&mut self, input: MediaProcessorInput) -> crate::Result<()> {
        let input_stream = self
            ...
            .ok_or_else(|| {
                crate::Error::new(format!("unknown input stream: {:?}", input.stream_id))
            })?;
    ...
    fn process_output(&mut self) -> crate::Result<MediaProcessorOutput> {
        loop {
            ...
            if gap > TIMESTAMP_GAP_THRESHOLD {
                ...
                let duration = self.next_output_duration();
                if self.last_mixed_frame.is_some() {
                    if let Some(flushed) = self.extend_gap_hold(duration) {
                        return Ok(MediaProcessorOutput::video_frame(
                            self.output_stream_id,
                            flushed,
                        ));
                ...
                // last_mixed_frame が無いときは延長できないので、Scheduler 経路と同じく mix() へ進む
            }
