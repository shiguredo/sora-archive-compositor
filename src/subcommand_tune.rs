...
use crate::{
    ...
    tune::{SearchSpace, TrialValues, Tuner},
};
...
impl Args {
    fn parse(raw_args: &mut noargs::RawArgs) -> noargs::Result<Self> {
        Ok(Self {
            layout_file_path: noargs::opt("layout-file")
                .short('l')
                .ty("PATH")
                .default("SORA_ARCHIVE_COMPOSITOR_REPO/layout-examples/tune-libvpx-vp9.jsonc")
            ...
            search_space_file_path: noargs::opt("search-space-file")
                .short('s')
                .ty("PATH")
                .default("SORA_ARCHIVE_COMPOSITOR_REPO/search-space-examples/full.jsonc")
                .doc("探索空間定義ファイル（JSON）のパスを指定します")
                .take(raw_args)
                .then(crate::arg_utils::parse_non_default_opt)?,
            tune_working_dir: noargs::opt("tune-working-dir")
                .ty("PATH")
                .default("ROOT_DIR/tune/")
    ...
    fn tune_working_dir(&self) -> PathBuf {
        ...
        self.tune_working_dir
            .clone()
            .unwrap_or_else(|| self.root_dir.join("tune/"))
    }
}
...
fn run_trial_evaluation(
    ...
    // 自分自身の vmaf サブコマンドを呼ぶので、PATH 上の別バイナリを誤って拾わないよう
    // current_exe() で実行中のバイナリを直接指定する。
    let compositor_exe = std::env::current_exe()
        .map_err(|e| crate::Error::new(format!("failed to resolve current executable: {e}")))?;
    let mut cmd = Command::new(&compositor_exe);
    cmd.arg("vmaf")
        .arg("--layout-file")
    ...
    let result = cmd
        ...
        .map_err(|e| {
            crate::Error::new(format!(
                "failed to execute `$ sora-archive-compositor vmaf` command: {e}"
            ))
        })
        .and_then(|output| {
            if !output.status.success() {
                return Err(crate::Error::new(
                    "`$ sora-archive-compositor vmaf` command failed",
                ));
            }
            Ok(output)
        });
    ...
    // TODO(sile): compose コマンドを実行して所要時間を計測することを検討する
    //
    // 今は `vmaf` コマンドの所要時間を使って最適化を行っているが、
    ...
    //
    // そのため `compose` を使って所要時間を計測した方が、実際の値に近くなる。
    // ただし、その場合、（余計な合成処理が増えるので）最適化にかかる時間が長くなる、というデメリットがある。
    // また、`vmaf` での所要時間計測方法が多少不正確だとしても、最適化の用途では通常は問題ない
...
fn display_best_trials_if_updated(
    ...
    for trial in best_trials {
        ...
        eprintln!(
            "    $ sora-archive-compositor compose -l {} {}",
            layout_file_path.display(),
