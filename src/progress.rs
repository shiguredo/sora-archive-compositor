...
impl ProgressBar {
    ...
    /// 表示行の先頭に付けるラベルを設定する (何を進捗させているかの説明用)
    ...
    fn render_line(&self, elapsed: Duration) -> String {
        ...
        match self.kind {
            ProgressKind::Time => format!(
                "{label_text}[{elapsed_text} (ETA: {eta_text})] [{bar_text}] complete {percent}% of {len}s total output duration",
                len = self.total
            ),
            ProgressKind::Frame => format!(
                "{label_text}[{elapsed_text} (ETA: {eta_text})] [{bar_text}] complete {percent}% of {len} total frames",
                len = self.total
            ),
        }
    }
...
mod tests {
    ...
    #[test]
    fn render_line_includes_label_when_set() {
        // ラベルを設定すると表示行の先頭に付与される
        let mut bar = ProgressBar::new(100, ProgressKind::Frame).with_label("encoding frames");
        bar.set_position(50);
        let line = bar.render_line(Duration::from_secs(10));
        assert!(line.starts_with("encoding frames: "));
        assert!(line.contains("complete 50% of 100 total frames"));
    }

    #[test]
    fn render_line_omits_label_by_default() {
        // ラベル未設定の場合は表示行の先頭に何も付与されない
        let mut bar = ProgressBar::new(100, ProgressKind::Frame);
        bar.set_position(50);
        let line = bar.render_line(Duration::from_secs(10));
