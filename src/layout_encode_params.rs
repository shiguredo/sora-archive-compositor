    ...
    fn try_from(value: nojson::RawJsonValue<'text, 'raw>) -> Result<Self, Self::Error> {
        ...
        for (key, value) in value.to_object()? {
            match &*key.to_unquoted_string_str()? {
                ...
                unknown if unknown.ends_with("_encode_params") => {
                    // エンコードパラメーターの未知キーは警告して無視する
                    // (typo や廃止されたキーに気付けるようにするため)
                    tracing::warn!("ignored unknown layout encode param: {unknown}");
                }
                // エンコードパラメーター以外のキー (デコード系や layout 系) は無視する
                _ => {}
            }
        }
        Ok(params)
