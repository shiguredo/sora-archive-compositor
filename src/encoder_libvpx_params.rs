...
pub fn parse_vp8_encode_params(
    ...
    // width / height / image_format はエンコーダー生成時に上書きするためのプレースホルダー。
    // ここで指定した値は encoder_libvpx.rs 側で width / height を差し替える際にリセットされる
    ...
    params.warn_unknown_keys("libvpx_vp8");
...
pub fn parse_vp9_encode_params(
    ...
    update_vp9_encode_params(&params, &mut config)?;
    // 未知のキーはデフォルトレイアウト側では出さず、ユーザーレイアウト側でのみ警告する
    params.warn_unknown_keys("libvpx_vp9");
...
fn update_vp8_encode_params(
    ...
    let shiguredo_libvpx::CodecConfig::Vp8(vp8_config) = &mut config.codec else {
        // parse_vp8_encode_params 側で Vp8 として初期化しているので、この分岐は起きない
        unreachable!("libvpx encoder config is not VP8");
    };
...
fn update_vp9_encode_params(
    ...
    let shiguredo_libvpx::CodecConfig::Vp9(vp9_config) = &mut config.codec else {
        // parse_vp9_encode_params 側で Vp9 として初期化しているので、この分岐は起きない
        unreachable!("libvpx encoder config is not VP9");
    };
