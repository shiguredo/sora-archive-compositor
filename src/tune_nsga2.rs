...
use crate::json::{JsonNumber, JsonObjectMemberPath, JsonValue};
...
//
// 本クレートが使うのは 2 目的 (合成時間 minimize / VMAF 平均 maximize) の多目的最適化のみ。
...
/// サンプリングになる。本クレートでは 1 試行が合成 + VMAF 評価で高コストなため、限られた試行数でも
...
/// 支配されず rank 0 (パレートフロント) に居座る。本クレートでは VMAF と合成時間とも有限値しか
...
fn tournament_select(ranks: &[usize], distances: &[f64]) -> crate::Result<usize> {
    ...
    let b = if n <= 1 {
        ...
        let mut b = tune_rng::gen_index(n)?;
        while b == a {
            b = tune_rng::gen_index(n)?;
        }
        b
    };
...
/// 順序を返す (`Less` なら a 優先、`Greater` なら b 優先、`Equal` なら rank と距離とも同値)。
    ...
    match dist {
        ParameterDistribution::Numeric { min, max } => match (min, max) {
            ...
            (JsonNumber::Integer(lo), JsonNumber::Integer(hi)) => {
                Ok(JsonValue::Integer(tune_rng::gen_range_i64(*lo, *hi)?))
            }
            _ => {
                let lo = min.to_f64();
                let hi = max.to_f64();
                Ok(JsonValue::Float(lo + tune_rng::gen_unit_f64()? * (hi - lo)))
            }
        },
        ParameterDistribution::Categorical(choices) => {
            ...
            let idx = tune_rng::gen_index(choices.len())?;
...
pub fn generate_child(
    ...
    // 交叉
    let mut child = if tune_rng::gen_bool(CROSSOVER_PROB)? {
    ...
    for (path, dist) in &search_space.params {
        if tune_rng::gen_bool(mutation_prob)?
...
fn crossover_numeric(
    ...
    // (端点に確率質量が偏るが、極小レンジを特別扱いしない方針)。
    // なお polynomial mutation (mutate_one) は境界考慮版を使っている。
    let u = tune_rng::gen_unit_f64()?;
    let beta = if u <= 0.5 {
    ...
    let child = if tune_rng::gen_bool(0.5)? { c1 } else { c2 };
