...
use crate::tune_nsga2;
use crate::tune_nsga2::Individual;
use crate::tune_storage;
use crate::tune_storage::{LockGuard, TrialRecord, TrialResult};

// 2 目的 (合成時間 minimize / VMAF 平均 maximize) の多目的最適化を NSGA-II で行うモジュール。
//
// 最適化と試行履歴の永続化を本クレート内で完結させる。
// アルゴリズムの詳細は tune_nsga2 モジュール、履歴・ロックの永続化は tune_storage モジュールを参照。
...
impl TrialValues {
    ...
    pub(crate) fn to_objectives(&self) -> [f64; 2] {
...
impl Tuner {
    ...
    pub fn new(name: String, working_dir: PathBuf) -> crate::Result<Self> {
        ...
        let trials = tune_storage::load_trials(&jsonl_path)?;
    ...
    pub fn ask(&mut self, search_space: &SearchSpace) -> crate::Result<Trial> {
        ...
        let params = if individuals.len() < tune_nsga2::POPULATION_SIZE {
            // 世代 0 が埋まるまでは一様ランダムサンプリング (初期集団)
            tune_nsga2::sample_random(search_space)?
        } else {
            // 累積した成功個体から交叉 + 突然変異で子個体を生成する
            tune_nsga2::generate_child(search_space, &individuals)?
        };
    ...
    pub fn tell(&mut self, trial_number: usize, values: &TrialValues) -> crate::Result<()> {
        ...
        // 評価値は VMAF と合成時間とも常に有限なので、debug ビルドでのみ番兵として検査する。
        debug_assert!(
        ...
        tune_storage::append_trial(&self.jsonl_path, &record)?;
    ...
    fn compute_best_trials(&self) -> Vec<BestTrial> {
        ...
        let ranks = tune_nsga2::non_dominated_sort(&points);
