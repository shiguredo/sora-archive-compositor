...
impl OpusEncoder {
    pub fn new(bitrate: NonZeroUsize) -> crate::Result<Self> {
        // 2026.1.0 で Encoder::new は EncoderConfig を引数に取るようになった
        let mut config = shiguredo_opus::EncoderConfig::new(SAMPLE_RATE as u32, CHANNELS as u8);
        config.bitrate = Some(bitrate.get() as u32);
