...
#[test]
fn mix_three_sources_without_trim() -> sora_archive_compositor::Result<()> {
    // それぞれ期間が異なる三つのソース
...
#[test]
fn mix_three_sources_with_trim() -> sora_archive_compositor::Result<()> {
    // それぞれ期間が異なる三つのソース
...
#[test]
fn mix_three_sources_with_mixed_duration() -> sora_archive_compositor::Result<()> {
    // 100 ms のソースを三つ用意する
...
fn non_pcm_audio_input_error() -> sora_archive_compositor::Result<()> {
...
fn audio_data(
    ...
    let id = MediaStreamId::new(source.id.get().parse().expect("到達しないはず"));
...
fn next_mixed_data(mixer: &mut AudioMixer) -> sora_archive_compositor::Result<Arc<AudioData>> {
    mixer
        .process_output()?
        .expect_processed()
        .ok_or_else(|| sora_archive_compositor::Error::new("ミキサーがサンプルを出力しなかった"))?
        .1
        .expect_audio_data()
}
