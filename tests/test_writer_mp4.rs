...
fn write_audio_only_mp4() -> sora_archive_compositor::Result<()> {
...
fn write_video_only_mp4() -> sora_archive_compositor::Result<()> {
    let output_file_path = tempfile::NamedTempFile::new()?;
    let source = source(0, secs(0), secs(60));
    let layout = layout(&[], &[source.clone()]);
...
/// 映像のみで、途中でサンプルエントリーが変わる場合のテスト
///
/// 2 つ目のサンプルエントリーが来た時点でチャンクが強制的に分割され、
/// チャンク数が 2 になることを確認する。
#[test]
fn write_video_only_mp4_with_multiple_sample_entries() -> sora_archive_compositor::Result<()> {
    let output_file_path = tempfile::NamedTempFile::new()?;
    let source = source(0, secs(0), secs(60));
    ...
    // 1 秒尺の映像フレームを供給する
    // フレーム 30 から別のサンプルエントリーを載せる
    for i in 0..60 {
        let mut frame = video_frame(&source, i, secs(1));
        if i == 30 {
            frame.sample_entry = Some(SampleEntry::Unknown(UnknownBox {
                box_type: BoxType::Normal(*b"dum2"),
    ...
    // サンプルエントリーの変更でチャンクが強制的に分割される
    assert_eq!(stats.total_video_chunk_count.get(), 2);
    assert_eq!(stats.total_video_sample_count.get(), 60);
    assert_eq!(stats.total_video_track_duration.get(), secs(60));
...
fn write_video_and_audio_mp4() -> sora_archive_compositor::Result<()> {
    let output_file_path = tempfile::NamedTempFile::new()?;
...
#[test]
fn no_video_and_audio_mp4() -> sora_archive_compositor::Result<()> {
    let output_file_path = tempfile::NamedTempFile::new()?;
    let layout = layout(&[], &[]);
...
/// stts の u32 マイクロ秒上限を超える duration はエラーになる
#[test]
fn write_video_mp4_rejects_excessive_sample_duration() {
    let output_file_path = tempfile::NamedTempFile::new().expect("一時ファイル作成に失敗した");
    let source = source(0, secs(0), secs(60));
    let layout = layout(&[], std::slice::from_ref(&source));
    ...
    .expect("Mp4Writer の作成に失敗した");

    let mut frame = video_frame(&source, 0, secs(1));
    frame.duration = Duration::from_micros(u32::MAX as u64 + 1);
    writer
        .process_input(MediaProcessorInput {
            stream_id: VIDEO_STREAM_ID,
            sample: Some(MediaSample::Video(Arc::new(frame))),
        })
        .expect("フレーム入力に失敗した");

    let result = writer.process_output();
    assert!(
        result.is_err(),
        "上限超過 duration なのに成功した: {result:?}"
    );
    let message = result.unwrap_err().reason;
    assert!(
        message.contains("stts u32 limit"),
        "期待したエラーメッセージではない: {message}"
    );
}
...
fn layout(audio_sources: &[SourceInfo], video_sources: &[SourceInfo]) -> Layout {
    Layout {
        ...
        resolution: Resolution::new(16, 16).expect("到達しないはず"),
