...
#[test]
fn valid_resolutions() -> sora_archive_compositor::Result<()> {
...
#[test]
fn invalid_resolutions() -> sora_archive_compositor::Result<()> {
    let invalid_jsons = [
        // width が小さすぎる
        r#""15x20""#,
        // width が大きすぎる
...
#[test]
fn layout_without_resolution_specified() -> sora_archive_compositor::Result<()> {
...
#[test]
fn cell_width_and_cell_height_handling() -> sora_archive_compositor::Result<()> {
    let base_path = PathBuf::from(".");
...
#[test]
fn source_wildcard() -> sora_archive_compositor::Result<()> {
...
#[test]
fn source_path_outside_base_dir_error() -> sora_archive_compositor::Result<()> {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().reason.contains("outside the base dir"));

    Ok(())
}

#[test]
fn wildcard_excludes_sources_without_media_files() -> sora_archive_compositor::Result<()> {
...
#[test]
fn source_timestamps() -> sora_archive_compositor::Result<()> {
    // ソースのタイムスタンプ情報が適切に反映されているかどうかのテスト
    //
    // testdata/source_timestamps/*.webm は 0 バイトのプレースホルダである。
    // src/layout.rs の media_file_exists による存在チェックを通すためだけに置いており、
    // 本テストは JSON のタイムスタンプのみを検証する（webm 本体は読まない）。
    // 詳細は testdata/source_timestamps/README.md を参照すること。
    ...
    let source = layout
        .sources
        .get(&id)
        .ok_or_else(|| sora_archive_compositor::Error::new(format!("ソースがない: {id:?}")))?;
...
#[test]
fn trim() -> sora_archive_compositor::Result<()> {
    // trim 指定が適切に反映されているかどうかのテスト
    //
    // testdata/trim/*.webm は 0 バイトのプレースホルダである。
    // src/layout.rs の media_file_exists による存在チェックを通すためだけに置いており、
    // 本テストは JSON の時刻と trim 判定のみを検証する（webm 本体は読まない）。
    // 詳細は testdata/trim/README.md を参照すること。
