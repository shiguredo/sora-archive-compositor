...
```json
{
  ...
  "audio_sources_excluded": [ $SOURCE_FILE_NAME ],
...
Sora Archive Compositor は、この `archive.json` ファイルの中の以下の情報を参照します:
...
[`sora-archive-compositor compose`](./command_compose.md) コマンドなどの `ROOT_DIR` 引数で指定した値がベースパスとして扱われます。
...
- macOS 用にビルドされた Sora Archive Compositor（Apple Video Toolboxのエンコーダーが使用されます）
- [`sora-archive-compositor compose`](command_compose.md)  などのコマンドの引数で `--openh264` オプションが指定された場合
- nvcodec に対応してビルドされた Sora Archive Compositor（NVIDIA Video Codec SDKのエンコーダーが使用されます）
...
また `"AV1"` は常に指定可能ですが nvcodec に対応してビルドされた Sora Archive Compositor の場合には NVIDIA Video Codec SDKのエンコーダーが優先的に使用されます。
...
**注意**: `bitrate` フィールド（kbps 単位）も利用可能ですが、両方が指定された場合には `video_bitrate` が優先されます。
...
**非推奨**: この項目は Hisui との互換性のために残されています。`video_bitrate` の使用を推奨します。
