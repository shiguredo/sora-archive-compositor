# `sora-archive-compositor tune` コマンド

`sora-archive-compositor tune` コマンドは、映像のエンコードパラメーターの最適化を行うためのコマンドです。
...
`sora-archive-compositor tune` はそれを手軽に行えるようにするためのコマンドです。

## エンコードパラメーター調整の流れ

`sora-archive-compositor tune` を利用してエンコードパラメーターの調整を行う際の流れは、典型的には次のようになります。

1. ベースとするレイアウトファイルを決定します
   - 既に運用で使っているものがあるなら、それをベースにするのがいいです
   - そういったものがない場合には、Sora Archive Compositor リポジトリの `layout-examples/tune-*.jsonc` ファイルが参考になります
2. 上のレイアウトファイルの中で、調整対象となるパラメーターを決定します
   - レイアウトファイルの中で値が null の項目が調整対象となります（詳細は後述）
3. 探索で使用する録画ファイルを用意します
   - `sora-archive-compositor tune` は「パラメーターを変えつつ小規模な合成を行う」というプロセスを繰り返し実施します
   - そのため、その探索用の合成処理で使用する録画ファイルが必要となります
4. `sora-archive-compositor tune` を実行します
   - `sora-archive-compositor tune` は実際の運用環境（OS やマシン）に合わせた環境で実行するのが重要です
   - `sora-archive-compositor tune` を実行すると、`sora-archive-compositor compose` でそのまま利用可能な、調整後のレイアウトファイル群が生成されます
5. `sora-archive-compositor tune` によって見つかったレイアウトファイル群の中から、よさそうな候補をいくつか選択します
   - 詳しくは後述しますが、`sora-archive-compositor tune` では「エンコード時間（短いほどいい）」と「エンコード品質（高いほどいい）」という 2 つの指標で探索を行います
   - この 2 つの指標は一般にトレードオフ関係にあるため「単一なベストな結果」ではなく、時間か品質のどちらかの面で優れている、複数の結果が得られることになります
   - その中からどれを選択すべきかは実際のユースケース次第となります
6. `sora-archive-compositor compose` で、候補レイアウトファイルを使って実際に合成を行い、最終的に採用するレイアウトファイルを決定します
   - `sora-archive-compositor tune` では時間の節約のために部分的な合成しか行わず、また品質の評価が人間の感覚と一致しているとは限りません
...
```console
$ sora-archive-compositor tune -h
Sora Archive Compositor

Usage: sora-archive-compositor ... tune [OPTIONS] ROOT_DIR
...
Options:
  ...
  -l, --layout-file <PATH>       パラメータ調整に使用するレイアウトファイルを指定します [default: SORA_ARCHIVE_COMPOSITOR_REPO/layout-examples/tune-libvpx-vp9.jsonc]
  -s, --search-space-file <PATH> 探索空間定義ファイル（JSON）のパスを指定します [default: SORA_ARCHIVE_COMPOSITOR_REPO/search-space-examples/full.jsonc]
      --tune-working-dir <PATH>  チューニング用に使われる作業ディレクトリを指定します [default: ROOT_DIR/tune/]
      --name <NAME>              探索履歴の保存に使う名前を指定します（名前ごとに履歴が分かれます） [default: tune]
  ...
  -t, --trial-timeout <SECONDS>  各試行トライアルのタイムアウト時間（秒）を指定します（超過した場合は失敗扱い）
      --openh264 <PATH>          OpenH264 の共有ライブラリのパスを指定します [env: SORA_ARCHIVE_COMPOSITOR_OPENH264_PATH]
...
`sora-archive-compositor tune` は、内部の NSGA-II（多目的最適化アルゴリズム）を使って、以下の流れを繰り返します:
...
3. そのパラメーターセットを `sora-archive-compositor vmaf` コマンドで評価する
...
これらはすべて `sora-archive-compositor tune` が自動で行うため、利用者が中の動きを細かく意識する必要はありません。

### 最適化メトリクス

`sora-archive-compositor tune` コマンドは以下の2つの指標を同時に最適化します。
...
tune working dir:       /path/to/archive/RECORDING_ID/tune/
trials file:    /path/to/archive/RECORDING_ID/tune/tune.jsonl
name:     tune
target total trials:    100
tuning metrics: [Execution Time (minimize), VMAF Score Mean (maximize)]
tuning parameters (12):
  libvpx_vp9_encode_params.aq_mode:     {"min":0,"max":3}
  libvpx_vp9_encode_params.cpu_used:    {"min":0,"max":9}
  libvpx_vp9_encode_params.cq_level:    {"min":0,"max":63}
  libvpx_vp9_encode_params.deadline:    ["best","good","realtime"]
  libvpx_vp9_encode_params.lag_in_frames:       {"min":1,"max":25}
  libvpx_vp9_encode_params.max_quantizer:       {"min":30,"max":58}
  libvpx_vp9_encode_params.min_quantizer:       {"min":4,"max":29}
  libvpx_vp9_encode_params.noise_sensitivity:   {"min":0,"max":6}
  libvpx_vp9_encode_params.rate_control:        ["vbr","cbr","cq"]
  libvpx_vp9_encode_params.tile_columns:        {"min":0,"max":6}
  libvpx_vp9_encode_params.tile_rows:   {"min":0,"max":2}
  libvpx_vp9_encode_params.tune_content:        ["default","screen"]
...
=== EVALUATE PARAMETERS ===
$ "sora-archive-compositor" "vmaf" "--layout-file" "/path/to/trial-0/layout.jsonc" ...
...
Trial #0
  ...
  Parameters:
    libvpx_vp9_encode_params.aq_mode:     3
    libvpx_vp9_encode_params.cpu_used:    5
    libvpx_vp9_encode_params.cq_level:    32
    libvpx_vp9_encode_params.deadline:    "good"
    libvpx_vp9_encode_params.lag_in_frames:       16
    libvpx_vp9_encode_params.max_quantizer:       40
    libvpx_vp9_encode_params.min_quantizer:       20
    libvpx_vp9_encode_params.noise_sensitivity:   0
    libvpx_vp9_encode_params.rate_control:        "vbr"
    libvpx_vp9_encode_params.tile_columns:        2
    libvpx_vp9_encode_params.tile_rows:   0
    libvpx_vp9_encode_params.tune_content:        "default"
  Compose Command:
    $ sora-archive-compositor compose -l /path/to/trial-0/layout.jsonc /path/to/archive/RECORDING_ID/

...
```

`sora-archive-compositor tune` コマンドの出力には、以下のような情報が含まれています。
- `====== INFO ======`
  - 探索（最適化）の基本情報が表示されます
...
- `====== BEST TRIALS (sorted by execution time) ======`
  ...
  - 表示タイミングは以下の通りです
    - `sora-archive-compositor tune` コマンドを実行して、最初のトライアルの完了後
    - 新しい最適解が発見されて、最適解集合が更新された後
    - `sora-archive-compositor tune` コマンドが指定のトライアル回数の実行を完了して終了する時
...
もし、この `sora-archive-compositor compose` の実行結果が期待通りのものであれば、このトライアルのレイアウトファイルを、
...
`sora-archive-compositor tune` コマンドで使用するレイアウトファイルは基本的には通常のものと同様です。
ただし、JSON オブジェクトのメンバーの値が `null` の場合には、
それが NSGA-II によって提案された値に置換された上で `sora-archive-compositor vmaf` コマンドに渡される点が異なります。
...
`sora-archive-compositor tune` は以下の 3 つの形式の範囲定義をサポートしています。
...
`sora-archive-compositor` リポジトリには各コーデック・エンコーダー毎に参考にできるレイアウトファイルが用意されています。
...
なお、`sora-archive-compositor tune` では音声の合成は行われないので、音声関連の項目がレイアウトファイルに含まれていても、単に無視されます。
...
`sora-archive-compositor tune` の仕組みは柔軟なので、やろうと思えば レイアウト JSON 内のほぼ全ての項目を探索対象に含めることができます。

しかし、以下のパラメーターは探索対象から除外して、事前に決めた固定値を使用すべきです。
- 解像度 (`resolution`)
  - `sora-archive-compositor vmaf` コマンドでは「合成後の画像（参照画像）」と「それをエンコードした後の画像」を比較して VMAF スコアを計算します
...
- コーデック (`video_codec`)
  - `sora-archive-compositor vmaf` コマンドでは VMAF スコアを計算するために、合成画像をエンコードした後に再度デコードして生画像を取得しています
...
`sora-archive-compositor tune` コマンドは、処理時間と映像品質のトレードオフを考慮して、
...
トライアルで使用されたレイアウトファイルを使って、実際に `sora-archive-compositor compose` コマンドを実行するのが確実です。

### 以前の探索の続きから再開したい場合

`ROOT_DIR` 引数や `--name` オプションの値を変えずに `sora-archive-compositor tune` コマンド
...
なお、旧バージョンで使用していた Optuna の SQLite ファイル（`optuna.db`）は引き継げません。
JSON Lines 形式での探索を新たに開始してください。

### 探索空間やレイアウトファイルを変更して探索を行いたい場合

上述の通り、デフォルトでは `sora-archive-compositor tune` コマンドは前回の探索結果を引き継ぎます。
...
各トライアルの実行時には、実際に実行された `sora-archive-compositor vmaf` コマンドがターミナルに表示されるので、
それを参考にして、問題となったレイアウトファイルを修正しつつ `sora-archive-compositor vmaf` コマンドを直接叩けば、
具体的にどのパラメーターの組み合わせが失敗の原因になっているのかを特定できます。

また、もし常に失敗するようであれば、
Sora Archive Compositor や依存ライブラリー（例えば OpenH264 の動的ライブラリー）のビルドに失敗している可能性が高いです。
