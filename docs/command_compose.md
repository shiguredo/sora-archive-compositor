# `sora-archive-compositor compose` コマンド

`sora-archive-compositor compose` コマンドは、Sora が保存した録画ファイルを合成するためのコマンドです。
...
```console
$ sora-archive-compositor compose -h
Sora Archive Compositor

Usage: sora-archive-compositor ... compose [OPTIONS] ROOT_DIR
...
Options:
  ...
  -l, --layout-file <PATH>     合成に使用するレイアウトファイルを指定します [env: SORA_ARCHIVE_COMPOSITOR_LAYOUT_FILE_PATH] [default: SORA_ARCHIVE_COMPOSITOR_REPO/layout-examples/compose-default.jsonc]
  ...
  -s, --stats-file <PATH>      合成中に収集した統計情報 (JSON) を保存するファイルを指定します
      ...
      --fdk-aac <PATH>         FDK-AAC の共有ライブラリのパスを指定します [env: SORA_ARCHIVE_COMPOSITOR_FDK_AAC_PATH]
  -P, --no-progress-bar        指定された場合は、合成の進捗を非表示にします
  -T, --thread-count <INTEGER> 合成処理に使用するワーカースレッド数を指定します [env: SORA_ARCHIVE_COMPOSITOR_THREAD_COUNT] [default: 1]
```
...
```console
$ sora-archive-compositor compose -l layout-examples/compose-default.jsonc /path/to/archive/RECORDING_ID/
  [00:00:09] [########################################] 27/27s (0s)
{
...
Sora Archive Compositor でのエンコードおよびデコード時に利用可能なコーデックは、様々な要因によって変ります。
実際の環境でどのコーデックが使用できるかは [`sora-archive-compositor list-codecs`](command_list_codecs.md) コマンドで確認できます。

### 最適な映像エンコードパラメーターの決定方法

Sora Archive Compositor ではエンコーダー毎に、細かくエンコードパラメーターが指定できるようになっています。

具体的にどのパラメーターが最適かは、ユーザーの要件（例えば、映像品質を優先したいのか、それとも合成速度を優先したいのか）や実行環境によって変ります。

[`sora-archive-compositor tune`](command_tune.md) コマンドを利用することで、エンコーダーに詳しくなくても、最適なパラメーターの探索を行いやすくなっているので、ぜひ試してみてください。

### マルチスレッドで合成を行う方法

Sora Archive Compositor はデフォルトではシングルスレッドで合成処理を実行しますが、`--thread-count` オプションを指定することでマルチスレッドで処理させることができます。
もし一度に実行する Sora Archive Compositor プロセスがひとつだけで、できるだけ合成処理時間を短くしたい場合には、このオプションの値に CPU の物理コアの数を指定してみてください。

なお、この `--thread-count` オプションが制御するのは Sora Archive Compositor 自体のワーカースレッド数のみです。
