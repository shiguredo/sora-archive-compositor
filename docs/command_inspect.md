# `sora-archive-compositor inspect` コマンド（開発者向け）

`sora-archive-compositor inspect` コマンドは、録画ファイルの詳細情報を取得するためのコマンドです。
このコマンドは主にデバッグやファイル分析に使用されます。

## 使用方法

```console
$ sora-archive-compositor inspect -h
Sora Archive Compositor

Usage: sora-archive-compositor ... inspect [OPTIONS] INPUT_FILE

Example:
  $ sora-archive-compositor inspect /path/to/archive.mp4
...
```console
$ sora-archive-compositor inspect /path/to/archive.mp4 | head
{
