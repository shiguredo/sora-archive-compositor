...
Sora Archive Compositor では H.264、H.265、VP8、VP9、AV1 のデコードに利用できます。
...
- nvcodec に対応してビルドされた Sora Archive Compositor
  - Ubuntu x86_64 向けのビルド済みバイナリは nvcodec が有効になっています（CUDA がない環境では実行時に無効になります）
  - 自前でビルドした Sora Archive Compositor で nvcodec を有効にする場合には [ビルド方法](./build.md) をご参照ください
...
- `reconfigure_enabled` (真偽値): ストリーム中の解像度変化を `cuvidReconfigureDecoder` で再構成するかどうか
  - デフォルト値: `false` (推奨)
  - `false` の場合、シーケンス変更ごとに decoder を破棄して再作成します
  - `true` の場合、現在の decoder session の上限以内の解像度変化を再構成で処理します
  - `true` は `max_display_delay > 0` と組み合わせられません
