...
Sora Archive Compositor を最大限活用するためには、これらを適切に指定することが重要です。

実際に利用可能なコーデックは、Sora Archive Compositor のビルド方法や実行環境で変わりますが、
[`sora-archive-compositor list-codecs`](./command_list_codecs.md) コマンドで一覧を取得することができます。

なお [`sora-archive-compositor tune`](./command_tune.md) コマンドを利用することで、
...
- macOS 用にビルドされた Sora Archive Compositor（Apple Audio Toolboxの AAC エンコーダーが使用されます）
- FDK-AAC を有効にしてビルドされた Sora Archive Compositor（参考: [build.md](build.md)）
...
- **macOS 用にビルドされた Sora Archive Compositor**: Apple Video Toolbox の H.264 エンコーダーが使用されます
- **OpenH264 オプション指定時**: [`sora-archive-compositor compose`](command_compose.md) などのコマンドで `--openh264` オプションや `SORA_ARCHIVE_COMPOSITOR_OPENH264_PATH` 環境変数が指定された場合
- **nvcodec 対応時**: nvcodec に対応してビルドされた Sora Archive Compositor では NVIDIA Video Codec SDK の H.264 エンコーダーが使用されます

`"H265"` は、以下のいずれかの条件を満たしている場合にのみ指定可能です：

- **macOS 用にビルドされた Sora Archive Compositor**: Apple Video Toolbox の H.265 エンコーダーが使用されます
- **nvcodec 対応時**: nvcodec に対応してビルドされた Sora Archive Compositor では NVIDIA Video Codec SDK の H.265 エンコーダーが使用されます

`"AV1"` は、以下のいずれかの条件を満たしている場合にのみ指定可能です：

- **SVT-AV1 対応時**: SVT-AV1 に対応してビルドされた Sora Archive Compositor では SVT-AV1 エンコーダーが使用されます
- **nvcodec 対応時**: nvcodec に対応してビルドされた Sora Archive Compositor では NVIDIA Video Codec SDK の AV1 エンコーダーが使用されます

なお自前で Sora Archive Compositor のビルドを行う場合には、nvcodec はデフォルトでは無効になっています。
有効にする方法は [ビルド方法](build.md) をご参照ください。
Ubuntu x86_64 向けのビルド済みバイナリでは nvcodec が有効になっています（CUDA がない環境では実行時に無効になります）。
...
`bitrate` フィールド（kbps 単位）も利用可能ですが、両方が指定された場合には `video_bitrate` が優先されます。

## 映像エンコーダー固有のパラメーターセットの指定

Sora Archive Compositor は映像のエンコーダーとして、以下をサポートしています:
...
- `entropy_coding_mode` (文字列): エントロピー符号化モード
  - デフォルト値: `"cavlc"`
  - 指定可能な値: `"cavlc"`（高速）, `"cabac"`（高効率）

- `entropy_coding` (真偽値): エントロピー符号化の有効化（CABAC/CAVLC選択）
  - **非推奨（互換用）**: 2026.1.0 で `entropy_coding_mode` に置き換えられました。`true` は `"cabac"`、`false` は `"cavlc"` として扱われます
  - デフォルト値: `false`
...
- `aq_mode` (整数値): 適応量子化モード
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 0 〜 2
  - 0=CQP、1=variance、2=CRF

- `sharpness` (整数値): シャープネスレベル
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: -7 〜 7

- `rtc` (真偽値): リアルタイムコミュニケーションモード
  - デフォルト値: なし（省略可能）

- `tune` (文字列): 品質チューニングメトリクス
  - デフォルト値: なし（省略可能）
  - 指定可能な値: `"vq"`, `"psnr"`, `"ssim"`, `"iq"`, `"ms_ssim"`

- `intra_refresh_type` (文字列): イントラリフレッシュタイプ
  - デフォルト値: なし（省略可能）
  - 指定可能な値: `"fwdkf_refresh"`, `"kf_refresh"`

- `screen_content_mode` (整数値): スクリーンコンテンツモード
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 0 〜 3
  - 0=なし、1=Block Copy + Palette、2=content adaptive、3=content adaptive (anti-alias aware)

- `lossless` (真偽値): ロスレスモード
  - デフォルト値: なし（省略可能）

- `ac_bias` (浮動小数点数): AC バイアス
  - デフォルト値: なし（省略可能）

- `level_of_parallelism` (整数値): 並列度
  - デフォルト値: なし（省略可能）
...
- `max_bit_rate` (整数値): 最大ビットレート（bps）
  - デフォルト値: なし（省略可能）

- `vbr_min_section_pct` (整数値): VBR 最小セクション率（%）
  - デフォルト値: なし（省略可能）

- `vbr_max_section_pct` (整数値): VBR 最大セクション率（%）
  - デフォルト値: なし（省略可能）

- `mbr_over_shoot_pct` (整数値): 最大ビットレート超過許可率（%）
  - デフォルト値: なし（省略可能）

- `recode_loop` (整数値): リコードループ制御
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 0 〜 4
  - 0=無効、1=KF+最大帯域超過、2=KF/ARF/GF のみ、3=全フレーム、4=プリセット依存

- `starting_buffer_level_ms` (整数値): 開始時バッファレベル（ミリ秒）
  - デフォルト値: なし（省略可能）

- `optimal_buffer_level_ms` (整数値): 最適バッファレベル（ミリ秒）
  - デフォルト値: なし（省略可能）

- `maximum_buffer_size_ms` (整数値): 最大バッファサイズ（ミリ秒）
  - デフォルト値: なし（省略可能）
...
- `intra_period_length` (整数値): イントラフレーム間隔
  - デフォルト値: 300
  - 指定可能な範囲: 1 以上 (`i32` に収まる値)
  - 省略するとデフォルトレイアウトの `300` が使われます
  - Hisui 2025.3.3 の `-1` (無制限) は指定できません

- `hierarchical_levels` (整数値): 階層レベル数
  - デフォルト値: 3
  - 指定可能な範囲: 0 〜 5
  - テンポラルレイヤー数は `hierarchical_levels + 1` です
...
- `look_ahead_distance` (整数値): 先読みフレーム数
  - デフォルト値: 13
  - 指定可能な範囲: 0 以上 (`u32` に収まる値)

- `sframe_dist` (整数値): スイッチフレーム間隔
  - デフォルト値: なし（省略可能）

- `sframe_mode` (整数値): スイッチフレームモード
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 1 〜 4
  - 1=STRICT、2=NEAREST、3=FLEXIBLE、4=DEC_POSI

- `sframe_qp` (整数値): スイッチフレームの QP
  - デフォルト値: なし（省略可能）

- `sframe_qp_offset` (整数値): スイッチフレームの QP オフセット
  - デフォルト値: なし（省略可能）

- `gop_constraint_rc` (真偽値): GOP 制約付きレート制御
  - デフォルト値: なし（省略可能）

- `multiply_keyint` (真偽値): キーフレーム間隔の倍率適用
  - デフォルト値: なし（省略可能）

### 並列処理パラメーター

- `tile_columns` (整数値): タイル分割の列数 (log2 値)
  - デフォルト値: なし（省略時は分割なし）
  - 指定可能な範囲: 1 〜 4
  - タイル総数 `(1 << tile_columns) * (1 << tile_rows)` は 128 以下であること

- `tile_rows` (整数値): タイル分割の行数 (log2 値)
  - デフォルト値: なし（省略時は分割なし）
  - 指定可能な範囲: 1 〜 6
  - タイル総数 `(1 << tile_columns) * (1 << tile_rows)` は 128 以下であること
...
- `enable_dlf_flag` (整数値): デブロッキングループフィルター
  - デフォルト値: 0 (`false` も 0 として受け付ける)
  - 指定可能な範囲: 0 〜 2
  - 0=無効、1=有効、2=高精度
  - 互換のため真偽値も受け付けます (`true` は 1、`false` は 0)
...
- `enable_restoration_filtering` (整数値): 復元フィルタリング
  - デフォルト値: 0 (`false` も 0 として受け付ける)
  - 指定可能な範囲: -1 〜 1
  - -1=自動、0=無効、1=有効
  - 互換のため真偽値も受け付けます (`true` は 1、`false` は 0)

- `tf_strength` (整数値): テンポラルフィルター強度
  - デフォルト値: なし（省略可能）

- `enable_variance_boost` (真偽値): 分散ブーストの有効化
  - デフォルト値: なし（省略可能）

- `variance_boost_strength` (整数値): 分散ブースト強度
  - デフォルト値: なし（省略可能）

- `variance_octile` (整数値): 分散ブーストのオクタイル
  - デフォルト値: なし（省略可能）

- `variance_boost_curve` (整数値): 分散ブースト曲線
  - デフォルト値: なし（省略可能）
...
- `film_grain_denoise_apply` (整数値): フィルムグレインノイズ除去の適用
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 0 〜 1
  - 0=なし、1=フル適用

- `adaptive_film_grain` (真偽値): 適応フィルムグレインの有効化
  - デフォルト値: なし（省略可能）

### スーパーレゾリューション・リサイズパラメーター

- `superres_mode` (整数値): スーパーレゾリューションモード
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 0 〜 4

- `superres_denom` (整数値): スーパーレゾリューションの分母
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 8 〜 16
  - 8 はスケーリングなしです

- `superres_kf_denom` (整数値): キーフレームのスーパーレゾリューション分母
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 8 〜 16

- `superres_qthres` (整数値): スーパーレゾリューションの QP 閾値
  - デフォルト値: なし（省略可能）

- `superres_kf_qthres` (整数値): キーフレームのスーパーレゾリューション QP 閾値
  - デフォルト値: なし（省略可能）

- `superres_auto_search_type` (整数値): スーパーレゾリューションの自動探索タイプ
  - デフォルト値: なし（省略可能）

- `resize_mode` (整数値): リサイズモード
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 0 〜 3
  - 0=無効、1=固定、2=ランダム、3=動的

- `resize_denom` (整数値): リサイズの分母
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 8 〜 16

- `resize_kf_denom` (整数値): キーフレームのリサイズ分母
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 8 〜 16

### 高度な設定パラメーター

- `enable_tf` (整数値): Temporal Filtering
  - デフォルト値: 1 (`true` も 1 として受け付ける)
  - 指定可能な範囲: 0 〜 2
  - 0=無効、1=有効、2=適応的
  - 互換のため真偽値も受け付けます (`true` は 1、`false` は 0)
...
- `enable_qm` (真偽値): 量子化マトリクスの有効化
  - デフォルト値: なし（省略可能）

- `min_qm_level` (整数値): 最小量子化マトリクスレベル
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 0 〜 15

- `max_qm_level` (整数値): 最大量子化マトリクスレベル
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 0 〜 15

- `min_chroma_qm_level` (整数値): 最小クロマ量子化マトリクスレベル
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 0 〜 15

- `max_chroma_qm_level` (整数値): 最大クロマ量子化マトリクスレベル
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: 0 〜 15

- `max_tx_size` (整数値): 最大変換ブロックサイズ
  - デフォルト値: なし（省略可能）

- `enable_mfmv` (整数値): Motion Field Motion Vector
  - デフォルト値: なし（省略可能）
  - 指定可能な範囲: -1 〜 1
  - -1=自動、0=無効、1=有効

- `enable_dg` (真偽値): データ駆動型 GOP (Data-driven GOP)
  - デフォルト値: なし（省略可能）

- `avif` (真偽値): AVIF モード
  - デフォルト値: なし（省略可能）

- `startup_mg_size` (整数値): スタートアップマネジメントグループサイズ
  - デフォルト値: なし（省略可能）

- `startup_qp_offset` (整数値): スタートアップ QP オフセット
  - デフォルト値: なし（省略可能）

- `luminance_qp_bias` (整数値): 輝度 QP バイアス
  - デフォルト値: なし（省略可能）

- `qp_scale_compress_strength` (整数値): QP スケール圧縮強度
  - デフォルト値: なし（省略可能）

- `extended_crf_qindex_offset` (整数値): 拡張 CRF QINDEX オフセット
  - デフォルト値: なし（省略可能）
...
- `color_primaries` (文字列): 色原色
  - デフォルト値: なし（省略可能）
  - 指定可能な値: `"bt709"`, `"unspecified"`, `"bt470m"`, `"bt470bg"`, `"bt601"`, `"smpte240"`, `"generic_film"`, `"bt2020"`, `"xyz"`, `"smpte431"`, `"smpte432"`, `"ebu3213"`

- `transfer_characteristics` (文字列): 転送特性
  - デフォルト値: なし（省略可能）
  - 指定可能な値: `"bt709"`, `"unspecified"`, `"bt470m"`, `"bt470bg"`, `"bt601"`, `"smpte240"`, `"linear"`, `"iec61966"`, `"bt1361"`, `"srgb"`, `"bt2020_10bit"`, `"bt2020_12bit"`, `"pq"`, `"smpte428"`, `"hlg"`

- `matrix_coefficients` (文字列): マトリクス係数
  - デフォルト値: なし（省略可能）
  - 指定可能な値: `"identity"`, `"bt709"`, `"unspecified"`, `"fcc"`, `"bt470bg"`, `"bt601"`, `"smpte240"`, `"ycgco"`, `"bt2020_ncl"`, `"bt2020_cl"`, `"smpte2085"`, `"chromat_ncl"`, `"chromat_cl"`, `"ictcp"`

- `color_range` (文字列): 色域
  - デフォルト値: なし（省略可能）
  - 指定可能な値: `"studio"`, `"full"`

- `chroma_sample_position` (文字列): クロマサンプル位置
  - デフォルト値: なし（省略可能）
  - 指定可能な値: `"unknown"`, `"vertical"`, `"colocated"`

- `mastering_display` (オブジェクト): マスタリングディスプレイ情報
  - デフォルト値: なし（省略可能）
  - 指定可能なフィールド: `r`, `g`, `b`, `white_point`（各 [x, y] 色度座標）、`max_luminance`、`min_luminance`

- `content_light_level` (オブジェクト): コンテンツ輝度レベル
  - デフォルト値: なし（省略可能）
  - 指定可能なフィールド: `max_cll`, `max_fall`

### エンコーダー固有設定パラメーター

- `fast_decode` (整数値): デコーダー速度最適化
  - デフォルト値: 0 (`false` も 0 として受け付ける)
  - 指定可能な範囲: 0 〜 2
  - 0=無効、1=中程度、2=最大
  - 互換のため真偽値も受け付けます (`true` は 1、`false` は 0)
...
- `max_key_frame_interval_duration` (浮動小数点数): キーフレーム間隔の最大値（秒）
  - デフォルト値: なし（省略可能）
  - `max_key_frame_interval`（フレーム数）と併用でき、どちらか早い方が適用されます
...
- `data_rate_limits` (オブジェクト配列): データレートのハードリミット
  - デフォルト値: なし（省略可能）
  - 各要素は `bytes`（データ量）と `window_seconds`（時間窓）を持つオブジェクトの配列で指定します
  - 例:
    ```jsonc
    [
...
- CUDA対応のNVIDIA GPU
- 適切なNVIDIA GPUドライバー
- nvcodecに対応してビルドされたSora Archive Compositor
  - Ubuntu x86_64 向けのビルド済みバイナリは nvcodec が有効になっています（CUDA がない環境では実行時に無効になります）
