# `sora-archive-compositor vmaf` コマンド

`sora-archive-compositor vmaf` コマンドは、録画ファイルの合成結果のエンコード品質評価を行うためのコマンドです。
...
このコマンドは、主に [`sora-archive-compositor tune`](command_tune.md) コマンドと組み合わせて、
映像エンコードパラメーターの調整に利用することを想定しています。

## 使用方法

```console
$ sora-archive-compositor vmaf -h
Sora Archive Compositor

Usage: sora-archive-compositor ... vmaf [OPTIONS] ROOT_DIR
...
Options:
  ...
  -l, --layout-file <PATH>        合成に使用するレイアウトファイルを指定します [env: SORA_ARCHIVE_COMPOSITOR_LAYOUT_FILE_PATH] [default: SORA_ARCHIVE_COMPOSITOR_REPO/layout-examples/vmaf-default.jsonc]
...
なお、`sora-archive-compositor vmaf` コマンド実行後に生成される YUV ファイル群は、
