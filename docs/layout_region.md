...
**リージョン**は、Sora Archive Compositor の映像合成における基本的な配置単位です。
...
Sora Archive Compositor はその中の、以下の情報を参照しています:
...
```json
{
  ...
  ... 他の項目は Sora Archive Compositor は参照しないので省略 ...
}
```
...
Sora Archive Compositor の映像合成において、最終的な出力映像の解像度は複数の段階を経て決定されます。
...
なお、実際のリージョンサイズにはセルの間に挿入される内側の枠線の分が加算されるため、最終的なサイズは上の値よりも大きくなります。
この例では、内側の枠線（2 ピクセル × 1 本ずつ）が加算され、
最終的なリージョンサイズは `642x482` ピクセルとなります。

外側の枠線は、リージョンの外周に挿入されますが、トップレベル `resolution` が明示指定されて十分に大きい場合にのみ加算されます。
この例は `resolution` を指定していないため、外側の枠線は加算されません。
